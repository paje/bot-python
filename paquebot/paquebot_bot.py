...
class PaqueBot(Bot):
	...
	def __init__(self, token, crew, maindb, api_url_base=None, name=None, version=None, owner=None, timeout_s=20, poll_time_s=60):
		...
		self.dispatcher.add_handler(StartCommandHandler(callback=command.start_cb))

		# Handler for help command
		self.dispatcher.add_handler(HelpCommandHandler(callback=command.help_cb))
		...
		self.dispatcher.add_handler(MessageHandler(filters=Filter.text, callback=command.do_keepaneyeon))
		...
		self.dispatcher.add_handler(BotButtonCommandHandler(callback=command.buttons_answer_cb))
